...
const common = {
    trimBody: (body) => {
        for (var all in body) {
            if (
                typeof body[all] !== 'number' &&
                typeof body[all] !== 'object' &&
                typeof body[all] !== 'boolean'
            ) {
    ...
    generateRandomPassword: async () => {
        ...
        const randomLowercaseLetter = lowercaseLetters.charAt(
        ...
        const randomSpecialCharacter = specialCharacters.charAt(
        ...
        for (let i = 0; i < passwordLength; i++) {
            const randomChar = remainingCharacters.charAt(
