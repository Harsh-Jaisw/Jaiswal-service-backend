...
const faq = {
  addFaq: asyncHandler(async (req, res) => {
    ...
    if (newfaq) {
      ...
      return resp.cResponse(req, res, resp.BAD_REQUEST, con.faq.FAQ_NOT_CREATED);
    }
  }),
  ...
  getFaq: asyncHandler(async (req, res) => {
    ...
    const faq = await commonServices.getDataWithFilterAndPagination(req, tables.mf, filters, pagination, sort);

    if (faq) {
      return resp.cResponse(req, res, resp.SUCCESS, con.common.RECORD_SUCCESS, faq);
    } else {
      return resp.cResponse(req, res, resp.BAD_REQUEST, con.common.NO_RECORD);
    }
  }),
  ...
  updateFaq: asyncHandler(async (req, res) => {
    ...
    const faq = await commonServices.dynamicUpdate(req, tables.mf, updateData, {
      id: body.faq_id,
    });
    ...
    if (faq) {
      ...
      return resp.cResponse(req, res, resp.BAD_REQUEST, con.faq.FAQ_NOT_UPDATED);
    }
  }),
  ...
  deleteFaq: asyncHandler(async (req, res) => {
    ...
    if (faq) {
      ...
      return resp.cResponse(req, res, resp.BAD_REQUEST, con.faq.FAQ_NOT_DELETED);
    }
  }),
};
