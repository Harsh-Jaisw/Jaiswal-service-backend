...
const category = {
  addCategory: asyncHandler(async (req, res) => {
    ...
    if (req.fileDetails) {
      ...
      const existingCategory = await commonServices.readSingleData(req, tables.mc, '*', {
  ...
  updateCategory: asyncHandler(async (req, res) => {
    ...
    if (req.fileDetails) {
      ...
      const existingCategory1 = await commonServices.readSingleData(req, tables.mc, '*', {
        category_name: body.category_name,
      });

      if (existingCategory1.length > 1) {
        return resp.cResponse(req, res, resp.BAD_REQUEST, con.category.CATEGORY_EXISTS);
      }

      let result = await commonServices.dynamicUpdate(
        req,
        tables.mc,
        {
          category_name: req.body.category_name,
  ...
  updateCategoryStatus: asyncHandler(async (req, res) => {
    ...
    let result = await commonServices.dynamicUpdate(req, tables.mc, { is_active: req.body.status }, { id: body.category_id });
