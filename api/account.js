...
const account = {
    signUp: asyncHandler(async (req, res) => {
        const body = req.body
        let Result = await commonServices.readSingleData(
            req,
            tables.users,
            '*',
            {
    ...
    sendotp: asyncHandler(async (req, res) => {
        ...
        try {
            const loginResults = await commonServices.readSingleData(
                req,
                tables.users,
                '*',
                {email: body.email}
            ...
            if (
                ...
                loginResults[0].email.toLowerCase() === body.email.toLowerCase()
            ) {
                const newOtp = await helper.generateOtp()
                const updateData = {
                    otp: newOtp,
                }
                await commonServices.dynamicUpdate(
                    req,
                    tables.users,
                    updateData,
                    {
                ...
                return resp.cResponse(
                    ...
                    res,
                    resp.SUCCESS,
                    con.account.OTP_SENT
                )
            } else if (
                loginResults.length > 0 &&
                loginResults[0].status === 'Active'
            ) {
                ...
                await commonServices.dynamicInsert(
                    req,
                    tables.users,
                    insertData
                )
    ...
    verifyOtp: asyncHandler(async (req, res) => {
        ...
        const updateResults = await commonServices.dynamicUpdate(
            req,
            tables.users,
            updateData,
            {email: body.email, otp: body.otp}
        )
        const role = await commonServices.readSingleData(req, tables.mr, '*', {
            id: loginResults[0].role,
        })
        console.log(role)
        const tempData = {
        ...
        let regularToken = await helper.createToken(
            tempData,
            jwtConfig.jwtExpirySeconds,
            'login'
        )
        let refreshToken = await helper.createToken(
            tempData,
            jwtConfig.refreshTokenExpiry,
            'login'
        )

        let checkingDeviceToken = await commonServices.readAllData(
            req,
            tables.ud,
            '*',
            {
    ...
    logout: async (req, res) => {
        try {
            ...
            let devices = await commonServices.readAllData(
                req,
                tables.ud,
                '*',
                {
            ...
            if (deviceId.length != 0) {
                await commonServices.dynamicDeleteMultiple(req, tables.ud, {
                    id: deviceId,
                })
            }
    ...
    refreshToken: async (req, res) => {
        try {
            ...
            let token = await helper.createToken(
                tempData,
                jwtConfig.jwtExpirySeconds,
                'login'
            )
            ...
            return resp.cResponse(
                ...
                res,
                resp.SUCCESS,
                con.account.TOKEN_UPDATE_SUCCESS,
                {
                    token: token,
                    refreshToken: refreshToken,
    ...
    getProfile: asyncHandler(async (req, res) => {
        ...
        const loginResults = await commonServices.readSingleData(
            req,
            tables.users,
            'id,uid,firstName,lastName,email,profileImage,mobileNumber,level',
            {id: body.user_id}
        )
    ...
    updateProfile: asyncHandler(async (req, res) => {
        ...
        if (loginResults[0].id != userId) {
            return resp.cResponse(
                req,
                res,
                resp.FORBIDDEN_ERROR,
                con.common.PERMISSION_DENIED
            )
        }
        ...
        if (body.mobile_number) {
            updateData.mobileNumber = body.mobile_number
        }
        if (req.fileDetails) {
            const {original} = req.fileDetails
            updateData.profileImage = original
        }
