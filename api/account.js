...
const account = {
  ...
  sendotp: asyncHandler(async (req, res) => {
    ...
    try {
      const loginResults = await commonServices.readSingleData(req, tables.users, '*', {
        email: body.email,
      });
  ...
  verifyOtp: asyncHandler(async (req, res) => {
    ...
    if (Result.length == 0) {
      return resp.cResponse(req, res, resp.BAD_REQUEST, con.account.INVALID_OTP);
    }
    ...
    const updateResults = await commonServices.dynamicUpdate(req, tables.users, updateData, {
      email: body.email,
      otp: body.otp,
    });
  ...
  getProfile: asyncHandler(async (req, res) => {
    ...
    if (loginResults.length == 0) {
      return resp.cResponse(req, res, resp.BAD_REQUEST, con.account.NO_RECORD);
    }
    return resp.cResponse(req, res, resp.SUCCESS, con.account.RECORD_FOUND, loginResults[0]);
  }),
  ...
  updateProfile: asyncHandler(async (req, res) => {
    ...
    if (loginResults[0].id != userId) {
      return resp.cResponse(req, res, resp.FORBIDDEN_ERROR, con.common.PERMISSION_DENIED);
    }
    ...
    return resp.cResponse(req, res, resp.SUCCESS, con.account.PROFILE_UPDATED);
  }),
};
