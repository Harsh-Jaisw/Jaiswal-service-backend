...
const commonServices = {
  ...
  dynamicInsert: async (req, table, userDetail) => {
    const db = makeDb(config);
    try {
      let sql = `INSERT INTO ${table}(${Object.keys(userDetail).toString()}) VALUES (?)`;
      commonServices.loggerMessage(req, 'dynamicInsert', sql, [Object.values(userDetail)]);
  ...
  getDataWithFilterAndPagination: async (req, table, filters = {}, pagination = { page: 1, pageSize: 10 }, sort = null) => {
    const db = makeDb(config);
    try {
      // Base SQL queries
      ...
      // Log the queries
      commonServices.loggerMessage(req, 'fetchDataWithFilterAndPagination', baseSql, values);
      commonServices.loggerMessage(req, 'fetchDataWithFilterAndPagination', countSql, values.slice(0, values.length - 2)); // Exclude LIMIT and OFFSET for count query

      // Execute the count query
      const countResult = await db.query(countSql, values.slice(0, values.length - 2)); // Exclude LIMIT and OFFSET
