...
const commonServices = {
    ...
    getReportingMng: async (req, assigneeId) => {
        ...
        try {
            let sql =
                'SELECT u.user_id,u.user_name,ebd.reporting_manager as reportingManager FROM `users` as u LEFT JOIN employee_basic_detail as ebd ON u.user_id=ebd.user_id WHERE u.user_id= ? '
            commonServices.loggerMessage(req, 'getReportingMng', sql, [
                assigneeId,
            ])
    ...
    latestRecord: async (req, name, orgId) => {
        ...
        try {
            ...
            let result
            sql =
                'SELECT user_name,user_email from users where user_name=? AND org_id=? ORDER BY created_at DESC LIMIT 1'
            commonServices.loggerMessage(req, 'latestRecord', sql, [
                name,
                orgId,
            ])
    ...
    getSupportExecutiveList: async (req) => {
        ...
        try {
            let sql =
                "SELECT u.id,u.user_id,u.phone_number,ud.users_device_token FROM `users` as u LEFT JOIN users_devices as ud ON ud.user_id=u.id WHERE u.role =2 AND u.user_status='active' AND ud.users_device_token IS NOT NULL "
            commonServices.loggerMessage(
                req,
                'getSupportExecutiveList',
                sql,
                []
            )
    ...
    getNotificationForInquiryUserId: async (
        req,
        pageNumber,
        pageSize,
        userId
        ...
        try {
            ...
            let sql =
                "SELECT ni.product_id,p.product_name,(SELECT product_image_key FROM product_multiple_images WHERE product_id = p.product_id LIMIT 1) as product_multiple_images,ni.quantity,ni.unit_of_measurement,ni.created_date FROM `notification_inquiry` as ni LEFT JOIN products as p ON ni.product_id=p.id Where ni.user_id=? AND ni.notification_status='passed' "
            commonServices.loggerMessage(
                req,
                'getNotificationForInquiryUserId',
                sql,
                valueArray
            )
            ...
            let sqlRead =
                "SELECT (SELECT COUNT(*) FROM notification_inquiry WHERE is_read = '0' AND user_id =? AND notification_status='passed' ) AS unreadCount,(SELECT COUNT(*) FROM  notification_inquiry  WHERE is_read = '1' AND user_id = ? AND notification_status='passed') AS readCount "
            commonServices.loggerMessage(
                req,
                'getNotificationForInquiryUserId',
                sqlRead,
            ...
            result = await Promise.all(
                result.map(async (user) => {
                    if (
                        user.product_multiple_images &&
                        user.product_multiple_images.length > 0
                    ) {
                        const url = JSON.parse(user.product_multiple_images)
                        user.product_multiple_images =
                            await s3.createPreSignedUrl(url[0])
                    }
                    return user
                })
            )
            ...
            for (element of result) {
                ...
                element.request_received_time =
                    await commonHelper.DateTimeFormat(element.created_date)
                element.request_received_time_notification =
                    await commonHelper.timeAgo(element.created_date)
            }
    ...
    getNotificationForTaskUserId: async (req, pageNumber, pageSize, userId) => {
        ...
        try {
            let valueArray = [userId]
            let sql =
                "SELECT nt.id,nt.task_id,t.subject,t.task_details,l.primary_phone,l.secondary_phone,nt.notification_status,DATE_FORMAT(nt.created_date, '%Y-%m-%d') as created_date,DATE_FORMAT(t.follow_up_date, '%Y-%m-%d') as due_date,nt.is_read FROM `notification_task` as nt LEFT JOIN tasks as t ON t.id= nt.task_id LEFT JOIN leads as l ON l.id = nt.lead_id Where nt.user_id=? AND notification_status='passed' "
            commonServices.loggerMessage(
                req,
                'getNotificationForTaskUserId',
                sql,
                valueArray
            )
            let resultCount = await db.query(sql, valueArray)
            ...
            let sqlRead =
                "SELECT (SELECT COUNT(*) FROM notification_task WHERE is_read = '0' AND user_id =? AND notification_status='passed') AS unreadCount,(SELECT COUNT(*) FROM  notification_task  WHERE is_read = '1' AND user_id = ? AND notification_status='passed') AS readCount "
            commonServices.loggerMessage(
                req,
                'getNotificationForTaskUserId',
                sqlRead,
    ...
    getDataWithFilterAndPagination: async (
        req,
        table,
        filters = {},
        ...
        try {
            // Base SQL queries
            let baseSql = `SELECT * FROM ${table} WHERE 1`
            let countSql = `SELECT COUNT(*) AS totalCount FROM ${table} WHERE 1`
            ...
            commonServices.loggerMessage(
                req,
                'fetchDataWithFilterAndPagination',
                baseSql,
                values
            )
            commonServices.loggerMessage(
            ...
            const totalCount = countResult[0] ? countResult[0].totalCount : 0 // Handle case where countResult might be empty

            // Execute the data query
            const data = await db.query(baseSql, values)
